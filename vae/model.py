...
def recognition(input_images):
    with tf.variable_scope("recognition"):
        ...
        h1 = lrelu(h1)
        h2 = conv2d(h1, factor * 16, factor * 32, ker_size=5,stride=2, name="d_h2")  # 64x64x1 -> 32x32x32
        # h2 = batch_norm(h2, self.is_training)
        h2 = lrelu(h2)
        h3 = conv2d(h2, factor * 32, factor * 64, ker_size=5,stride=2, name="d_h3")  # 32x32x1 -> 16x16x16
        h3 = lrelu(h3)
        h4 = conv2d(h3, factor * 64, 1, ker_size=1,stride=1, name="d_h4")  # 32x32x1 -> 16x16x16
...
def classifier_net(z):
    with tf.variable_scope("classifier_net"):
        ...
        w_mean = dense(z, 16*16, 12*12, "classifier")
        w_mean = tf.nn.relu(w_mean)
        w_mean_2 = dense(w_mean, 12*12, 8*8, "classifier_2")
        w_mean_2 = tf.nn.relu(w_mean_2)
        w_mean_3 = dense(w_mean_2, 8 * 8, 4*4, "classifier_3")
